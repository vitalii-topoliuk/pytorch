  ...
  AOTInductorModelContainer(size_t num_models) {
    ...
    for (size_t i = 0; i < num_constants; i++) {
      ...
      C10_CUDA_CHECK(cudaMemcpy(
          constants_ptr,
          _binary_constants_bin_start + offset,
          data_size,
      ...
      constants_->emplace(
          std::move(name),
          at::from_blob(
              constants_ptr, size, at::device(at::kCUDA).dtype(dtype)));

      // Move GPU pointer to next aligned address.
      if (data_size % 64) {
        data_size = 64 + (data_size / 64) * 64;
      }
      constants_ptr = constants_ptr + data_size;
    }
  }
