...
namespace {
  static bool reduced_input(ScalarType input_t, ScalarType output_t) {
    return at::isReducedFloatingType(input_t) && output_t == kFloat;
  }

  static bool reduced_output(ScalarType input_t, ScalarType output_t) {
    return at::isReducedFloatingType(output_t) && input_t == kFloat;
  }
}

static bool reduced_float_type_copy(bool requires_conj, TensorIteratorBase &iter) {
  auto strides_out = iter.strides(0);
  auto strides_in = iter.strides(1);

  // Check whether input is in BFloat16/Half data type and output is in float data type, or
  // input is in float data type and output is in BFloat16/Half data type.
  // In addition, input and output need contiguous parts to utilize vectorization.
  return (!requires_conj && ((reduced_input(iter.dtype(1), iter.dtype(0)) &&
     sizeof(float) == strides_out[0] && (static_cast<int64_t>(elementSize(iter.dtype(1))) == strides_in[0] || strides_in[0] == 0)) ||
    (reduced_output(iter.dtype(1), iter.dtype(0)) &&
    static_cast<int64_t>(elementSize(iter.dtype(0))) == strides_out[0] && (sizeof(float) == strides_in[0] || strides_in[0] == 0))));
}
